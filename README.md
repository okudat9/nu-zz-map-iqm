...
(PRApplied 20, 034065). This is not a new method. It is a working implementation
for IQM hardware, where no equivalent tool was available, with the negative
control promoted from a good practice to a mandatory part of every output.
...
IQM's published calibration data reports gate fidelities, readout errors and
...
readout error and QND-ness, T1, T2 and T2-echo — and no ZZ metric of any kind. If you want to know the ZZ map of the chip you are running on, you
...
nothing real is being measured. Excited control qubits relax, preparation
errors shift both branches together, and the result is a smooth systematic
offset that looks exactly like a coupling.
...
measurement is telling you about your own apparatus, not the chip. The tool
says so and stops.
...
Both X and Y bases are measured, giving a complex amplitude
`z = ⟨X⟩ + i⟨Y⟩` for each control state. The differential phase

```
Δφ(τ) = arg( z_on · conj(z_ref) )
```

Its slope against τ is `2π·ν_ZZ`, extracted by weighted least squares with
...
**What the difference cancels, and what it does not.** Taking the argument of a
ratio removes *multiplicative* corruption — the target's own dephasing, contrast
loss, and any symmetric readout scaling divide out exactly. It does **not**
remove *additive* corruption, because `arg(z + c) ≠ arg(z)`. Two effects
therefore survive:

- **Asymmetric readout error.** Only the asymmetric part leaks; the symmetric
  part is a scale factor and cancels. On Garnet (`e01 ≈ 1.5%`, `e10 ≈ 0.8%`)
  the residual offset is about 0.7% of the amplitude, which propagates to
  roughly 0.1 kHz — below the resolution of the measurement. On a device with
  strongly asymmetric readout this would not be negligible.
- **Relaxation of the excited control.** When the control decays from |1⟩ to
  |0⟩ the coupling stops for the remainder of the delay, so less phase
  accumulates than the true `ν_ZZ` would give. This is a one-directional bias:
  **the reported values are lower bounds.** Using the measured calibration
  ...
  coupling.** The sign and the ordering of edges are unaffected; the magnitudes
  are underestimates by a factor that is similar across pairs on the same
  device.
...
Adding a device is three lines in `DEVICES` — URL, qubit count, median T2.
The coupling map, pair selection and τ range follow automatically.
...
```
        pair |   type   |       ν_ZZ (kHz) | contrast
      (0, 1) | adjacent |  -11.657 ± 2.463 |    0.193
      (9,12) | distant  |   -0.458 ± 0.627 |    0.510
...
avoid it. The numbers below are the actual Garnet measurement from the next
section.
...
|---|---|---|---|---|---|
| (0, 1) | 11.21 kHz | 0.018 | 0.035 | 0.070 | 0.176 |
| (2, 3) | 6.47 | 0.010 | 0.020 | 0.041 | 0.102 |
| (6, 11) | 5.36 | 0.008 | 0.017 | 0.034 | 0.084 |
| (4, 5) | 4.47 | 0.007 | 0.014 | 0.028 | 0.070 |
| (7, 8) | 2.01 | 0.003 | 0.006 | 0.013 | 0.032 |
...
Rank the edges and place idle-heavy parts of the circuit on the quiet ones.
On this Garnet snapshot, (7, 8) accumulates 5.6× less phase error than (0, 1).
No calibration data would have told you that.
...
|---|---|---|---|
| (0, 1) | −0.035 | −0.070 | −0.176 |
| (2, 3) | −0.020 | −0.041 | −0.102 |
| (7, 8) | −0.006 | −0.013 | −0.032 |
...
In the Garnet run, four edges fall in the 18–24% band (ranking) and (7, 8) at
...
Weighted mean **−0.444 ± 0.487 kHz**, i.e. 0.91σ from zero. A sign test on
four negative values gives a one-sided p of 0.063. Consistent with no coupling,
...
| pair | ν_ZZ (kHz) | τ\* (µs) |
|---|---|---|
| (0, 1) | −11.213 ± 2.463 | 24.8 |
| (2, 3) | −6.471 ± 1.309 | 42.9 |
| (6, 11) | −5.357 ± 1.268 | 51.9 |
| (4, 5) | −4.466 ± 0.809 | 62.2 |
| (7, 8) | −2.014 ± 2.075 | 137.9 |
...
fixed-frequency transmons with negative anharmonicity. Without the control
this would be indistinguishable from a systematic offset — which is precisely
the failure mode the control exists to catch.
...
map. IQM labels the same qubits from 1, so output `(0, 1)` is `QB1-QB2` and
`(7, 8)` is `QB8-QB9`. Both forms appear in the console output and the JSON.
...
~1.1 kHz, which is comparable to the couplings being measured.
...
| τ_max | Garnet (T2 = 8.6 µs) | Emerald (T2 = 15.1 µs) |
|---|---|---|
...
- **τ\* is reported for reference only.** For weakly coupled pairs it exceeds
  T2 and cannot be reached in practice.
...
If this tool contributes to your work, please cite it and get in touch —
the measurement has details worth discussing before you rely on the numbers.
...
Free for research and education. Commercial use requires permission —
...
- Sarovar, Proctor, Rudinger, Young, Nielsen & Blume-Kohout, *Detecting crosstalk
...
The measurement grew out of a series of Bell-state and ZZ experiments on IBM Heron
and IQM devices:
